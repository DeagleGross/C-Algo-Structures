...
#include <iostream> // needed for std::less
...
//        /** JUST FOR TESTING in rbtee_tests
//         * Checks if this.node fields are similar to left, parent and right that are passed
//         */
//        bool hasTheSameFields(const Node* left, const Node* parent, const Node* right) const
//        {
//            if (this->_left != left)
//                return false;
//
...
//        }
        ...
        /** ADDED BY MYSELF
         * \brief Возвращает истину, если у ноды есть папа и есть дедушка
         * и папа является правым ребенок для дедушки
         * Во всех остальных случаях (нет предка/нет дедушки или папа левый) — ложь.
         */
        bool isDadRightParent() const
        {
            if (!_parent)
                return false;
            ...
            return (_parent->_parent->_right == _parent);
        }
        ...
        /** ADDED BY MYSELF
         * \brief Находит брата вершины. Если его нет, то возвращает nullptr
         * @return nullptr если брата нет, в иных случаях - брата
         */
         Node* getBro(Node* me)
         {
            if (me->isRightChild())
                return me->_parent->_left;
            else
                if (me->_parent)
                    return me->_parent->_right;
                else
                    return nullptr;
         }
    ...
    protected:
            ...
            : _key(key), _left(left), _right(right), _parent(parent), _color(col)
        {
            ...
            // TODO дописать про родителя или сделать это в соответствующем евенте
        }
...
public:
     ...
     *  \returns узел элемента \c key, если он есть в дереве, иначе \c nullptr.
     */
    Node* findReal(const Element& key);

    /* ADDED BY MYSELF - from corman function
     * replaces one node with another
     */
    void replace(Node* past, Node* now)
    {
        // tree->root replacement
        if (!past->_parent)
            this->_root = now;
        else if (past->isLeftChild())
            past->_parent->_left = now;
        else
            past->_parent->_right = now;

        now->_parent = past->_parent;

//        // deleting former pointing
//        if (now->isRightChild())
//            past->_right = nullptr;
//        else
//            past->_left = nullptr;
    ...
    /** ADDED BY MYSELF
     * returns min in subtree from this node
     */
    Node* findMinInSubTree(Node* nd)
    {
        Node* ret = nd;
        if (ret->_left)
            ret = findMinInSubTree(ret->_left);
        else
            return ret;
    }
