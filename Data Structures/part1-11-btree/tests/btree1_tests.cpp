...
TEST_F(BTreeTest, MyOwnALLSearchTest)
{
    std::string& fn = getFn("AllSearch.xibt");
    ...
    // SETUP OF TREE READY
    const Byte* toSearchFifteen = new Byte(15);
    std::list<Byte*> keysF = std::list<Byte*>();
    int res1 = bt.searchAll(toSearchFifteen, keysF);
    EXPECT_EQ(res1, 2);

    const Byte* toSearchSixteen = new Byte(16);
    std::list<Byte*> keysS = std::list<Byte*>();
    int res2 = bt.searchAll(toSearchSixteen, keysS);
    EXPECT_EQ(res2, 3);

    const Byte* toSearch30 = new Byte(30);
    std::list<Byte*> keys3 = std::list<Byte*>();
    int res3 = bt.searchAll(toSearch30, keys3);
    EXPECT_EQ(res3, 1);

    const Byte* toSearch25 = new Byte(30);
    std::list<Byte*> keys25 = std::list<Byte*>();
    int res4 = bt.searchAll(toSearch25, keys25);
    EXPECT_EQ(res4, 1);
}

TEST_F(BTreeTest, MyOwnSearchTest)
{
    std::string& fn = getFn("MyOwnMainInsert.xibt");
    ...
    // SETUP OF TREE READY
    const Byte* toSearch = new Byte(4);
    Byte* rightRes = new Byte(4);
    Byte* got = bt.search(toSearch);
    EXPECT_EQ(*rightRes, *got); // find oke
    Byte* nullFind = bt.search(new Byte(6)); // null is searched oke
    EXPECT_EQ(nullFind, nullptr);
}

TEST_F(BTreeTest, MyOwnMainInsert)
{
    std::string& fn = getFn("MyOwnMainInsert.xibt");
...
//    return;
    // HERE IT IS OKAY
    // COMMENTED BECAUSE INSERT OF TWO SAME ELEMENTS -> DISASSEBMLY
...
TEST_F(BTreeTest, MyOwnSplit)
{
    std::string& fn = getFn("MyOwnSplit.xibt");

    ByteComparator comparator;
    FileBaseBTree bt(3, 1, &comparator, fn);

    FileBaseBTree::PageWrapper wp1(&bt);
    wp1.allocPage(3, false);
    *((UShort*)wp1.getKey(0)) = 1;
    *((UShort*)wp1.getKey(1)) = 10;
    *((UShort*)wp1.getKey(2)) = 20;
    ...
    *((UShort*)wp2.getKey(0)) = 3;
    *((UShort*)wp2.getKey(1)) = 4;
    *((UShort*)wp2.getKey(2)) = 5;
    *((UShort*)wp2.getKey(3)) = 6;
    *((UShort*)wp2.getKey(4)) = 7;
...
TEST_F(BTreeTest, MyInsertNonFull)
{
    std::string fn(TEST_FILES_PATH);
    fn.append("MyInsertNonFull.xibt");

    ByteComparator comparator;
    FileBaseBTree bt(5, 1, &comparator, fn);
    FileBaseBTree::PageWrapper wp(&bt);

    const Byte* a = new Byte(3);
    const Byte* b = new Byte(8);
    const Byte* c = new Byte(5);
    const Byte* d = new Byte(20);
    const Byte* e = new Byte(2);
    const Byte* f = new Byte(30);
    const Byte* g = new Byte(40);
    const Byte* h = new Byte(50);

    wp.allocPage(5, false);
    *(wp.getKey(0)) = 0x05; // 5
    *(wp.getKey(1)) = 0x11; // 17
    UShort am = wp.getKeysNum();
    ...
    wp.insertNonFull(f);
    wp.insertNonFull(g);
    EXPECT_THROW(wp.insertNonFull(h), std::domain_error);

    wp.writePage();
}

/// HERE COMES ALL PROVIDED TESTS
/// ------------------------------------------------------------------------------------------------------------------------------------------------
/// ------------------------------------------------------------------------------------------------------------------------------------------------
/// ------------------------------------------------------------------------------------------------------------------------------------------------
/// ------------------------------------------------------------------------------------------------------------------------------------------------
...
TEST_F(BTreeTest, Split1)
{
    ...
    *((UShort*)wp2.getKey(4)) = 0x0306;
    ...
    // выполняем собственно сплит
    EXPECT_THROW(wp1.splitChild(1), std::runtime_error);                          // курсор на 3 страницу
}
...
TEST_F(BTreeTest, Split2)
{
    ...
    *(wp2.getKey(2)) = 0x0A;
    ...
    // выполняем собственно сплит
    EXPECT_THROW(wp1.splitChild(2), std::runtime_error);                      // 2-й курсор на 3 страницу
...
TEST_F(BTreeTest, InsertNonFull1)
{
    ...
    wp1.allocPage(2, false);
    UShort amountBefore = wp1.getKeysNum();

    *(wp1.getKey(0)) = 0x05;
    *(wp1.getKey(1)) = 0x11;
    UShort amountAfter = wp1.getKeysNum();
    ...
    *(wp4.getKey(1)) = 0x15;
    ...
    //k = 0x04;                         // это норм
    k = 0x02;               
    EXPECT_THROW(wp1.insertNonFull(&k), std::domain_error);
}
