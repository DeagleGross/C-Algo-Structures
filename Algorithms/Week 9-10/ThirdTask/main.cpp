...
void getSubstrings(string& source, string& substring, vector<int>& res)
{
    ...
    // array of grani
    vector<int> arr = prefixFunction(substring);

    int nakop = 0;
    for (int i = 0; i < len_str; ++i)
    {
        // decreasing
        while (nakop > 0 && substring[nakop] != source[i])
            nakop = arr[nakop];

        // increasing
        if (substring[nakop] == source[i])
            ++nakop;

        // if reached length of substring
        if (nakop == len_sub)
        {
            // saving start of substring in string
            res.push_back(i - len_sub + 1);
            // decreasing
            nakop = arr[nakop - 1];
        }
    }
